...
class KDTree
{
  ...
  public:
    void copy(KDTree<Dim>& tocopy);
    void _clean(KDTreeNode *& subroot);
    void helper( vector<Point<Dim>>& newPoints, int start, int end, int dim, KDTreeNode *& subroot);
    //void copy(KDTree<Dim>& tocopy);
    Point<Dim> quickselect( vector<Point<Dim>>& list, int left, int right, int k, int dim);
    int partition( vector<Point<Dim>>& list, int left, int right, int pivotIndex, int dim);
    void _findNearestNeighbor(const Point<Dim>& query, KDTreeNode * subroot, Point<Dim>& curBest, int dim) const;
  //  void _clean(KDTreeNode *& subroot);
