                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
     ...
     TreeNode *topsingle = singleQueue.front();
     TreeNode *topmerge = mergeQueue.front();
     if (topsingle == NULL && topmerge == NULL) {
       return NULL;
     }
     int sigint = topsingle->freq.getFrequency();
     int merint = topmerge->freq.getFrequency();
     if (sigint > merint) {
       TreeNode *temp = mergeQueue.front();
       mergeQueue.pop();
       cout << "14." << endl;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for (unsigned long i = 0; i < frequencies.size(); i++) {
       TreeNode *toinsert = new TreeNode(frequencies[i]);
       singleQueue.push(toinsert);
     }
     while(mergeQueue.size() + singleQueue.size() != 1) {
       ...
       TreeNode *insertinmerge = new TreeNode(firstsmall->freq.getFrequency() + secondsmall->freq.getFrequency());
       insertinmerge->left = firstsmall;
       insertinmerge->right = secondsmall;
       mergeQueue.push(insertinmerge);
     }
     if (!singleQueue.empty()) {
     ...
     */
     while(bfile.hasBits()) {
       if (bfile.getNextBit() == true) {
        Frequency *frequ = new Frequency(bfile.getNextByte(), 0);
         TreeNode *anode = new TreeNode(*frequ);
        // anode->freq.character_ = getNextByte();
         delete frequ;
         return anode;
        // delete frequ;
          delete anode;
