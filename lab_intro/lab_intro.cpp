...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned int x = 0; x < image.width(); x++) {
    for (unsigned int y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      int xdistance = abs(int(centerX - x));
      int ydistance = abs(int(centerY - y));
      double edistance = sqrt(xdistance * xdistance + ydistance * ydistance);
      double reduction = 0.005 * edistance;

      if (edistance >= 160) {
        pixel.l = pixel.l * 0.2;
      } else {
      pixel.l = pixel.l * (1 - reduction);
    }
    }
  }
  return image;
