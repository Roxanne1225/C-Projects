...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string firs = word1;
    string secon = word2;
    std::transform(firs.begin(), firs.end(), firs.begin(), ::toupper);
    std::transform(secon.begin(), secon.end(), secon.begin(), ::toupper);

    if (dict.find(firs) == dict.end() || dict.find(secon) == dict.end()) {
      return false;
    }
    for(size_t i = 0; i < dict.find(firs)->second.size(); i++){
       if(dict.find(firs)->second[i] != dict.find(secon)->second[i]){
         return false;
       }
     }
     for(size_t i = 0; i < dict.find(secon)->second.size(); i++){
        if(dict.find(firs)->second[i] != dict.find(secon)->second[i]){
          return false;
        }
      }
