...
NimLearner::NimLearner(unsigned startingTokens) : g_(true, true) {
    ...
    for (; count < startingTokens; count++){
        g_.insertVertex("p1-" + std::to_string(count));
        g_.insertVertex("p2-" + std::to_string(count));
...
std::vector<Edge> NimLearner::playRandomGame() const {
 ...
 /* Your code goes here! */
    ...
    while (g_.getAdjacent(current).size() != 0) {
        vector<Vertex> posmoves = g_.getAdjacent(current);
        int next = rand() % posmoves.size();
        path.push_back(g_.getEdge(current, posmoves[next]));
        current = posmoves[next];
    }
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
 /* Your code goes here! */
 //1 for player1 wins and 2 for player2 wins
 int winner;
 ...
 if (winner == 1) {
   for (unsigned i = 0; i < path.size(); i++) {
     Edge edge = path[i];
     if (i % 2 != 0)   {
     int weight = g_.getEdgeWeight(edge.source, edge.dest) - 1;
